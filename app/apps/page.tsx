...
const APPS = [
  { id: "github", name: "GitHub", desc: "Repos, code, files & Actions", icon: Github, tile: "bg-koda-surface-2 text-koda-text", configured: true },
...
export default function AppsPage() {
  ...
  return (
    <div className="flex h-full w-full flex-col bg-koda-bg">
      {/* Header */}
      <header className="flex h-16 shrink-0 items-center justify-between border-b border-koda-border bg-koda-surface px-4 md:px-6">
        <button
          onClick={goBack}
          className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium text-koda-muted transition-colors hover:bg-koda-surface-2 hover:text-koda-text md:hidden"
        >
          <ChevronRight className="h-4 w-4 rotate-180" /> Back
        </button>
        <div className="flex-1 flex items-center gap-4 md:justify-center">
          <h1 className="text-lg font-semibold text-koda-text">Integrations</h1>
        </div>
        <div className="w-20 md:w-auto" />
      </header>

      {/* Tabs */}
      <div className="flex shrink-0 items-center gap-1 border-b border-koda-border px-4 md:px-6 bg-koda-surface/50">
        {["all", "connected", "available"].map((t) => (
          <button
            ...
            className={cn(
              ...
              tab === t ? "border-koda-accent text-koda-text" : "border-transparent text-koda-muted hover:text-koda-text"
            )}
          >
        ...
        <div className="relative">
          <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-koda-muted" />
          <input
            type="text"
            value={q}
            onChange={e => setQ(e.target.value)}
            placeholder="Search integrations…"
            className="w-64 pl-9 pr-3 py-1.5 rounded-lg border border-koda-border bg-koda-bg text-sm text-koda-text placeholder:text-koda-muted focus:border-koda-accent/50 focus:outline-none"
          />
        </div>
      </div>
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-6">
        {loading ? (
          <div className="flex h-64 items-center justify-center">
            <Loader2 className="h-8 w-8 animate-spin text-koda-accent" />
          </div>
        ) : (
          <>
            {showGithub && (
              ...
              >
                <span className={cn("flex h-11 w-11 shrink-0 items-center justify-center rounded-xl", gh?.connected ? "bg-emerald-500/15" : "bg-koda-surface-2 text-koda-text")}>
                  <Github className="h-5 w-5" />
                </span>
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-sm font-medium text-koda-text">GitHub</span>
            ...
            <div className="mt-5 grid grid-cols-1 gap-x-8 gap-y-1 sm:grid-cols-2 lg:grid-cols-3">
              {filtered.map((a) => (
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-sm font-medium text-koda-text">{a.name}</span>
                    <span className="block truncate text-xs text-koda-muted">{a.desc}</span>
                  </span>
            ...
            {filtered.length === 0 && !showGithub && (
              <p className="py-16 text-center text-sm text-koda-muted">No integrations match “{q}”.</p>
            )}
          </>
        )}
