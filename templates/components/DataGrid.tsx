...
export function DataGrid<T extends Record<string, any>>({
  ...
  return (
    <div className={cn("flex flex-col", className)} {...rest}>
      <div className="w-full overflow-x-auto rounded-xl border border-incogni-border">
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr className="border-b border-incogni-border bg-incogni-surface">
              {selectable && (
                <th className="w-10 px-3 py-3">
                  <button
                    ...
                    className={cn(
                      ...
                      allPageSelected
                        ? "border-incogni-accent bg-incogni-accent"
                        : "border-incogni-border hover:border-incogni-accent"
                    )}
                  >
              ...
              {columns.map((col) => (
                <th
                  key={col.key}
                  className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-incogni-muted"
                >
                  {col.label}
                </th>
              ))}
              ...
              {pageData.length === 0 ? (
                ...
                >
                  <td
                    ...
                    className="px-4 py-12 text-center text-incogni-muted"
                  >
                    {emptyMessage}
                  </td>
                </motion.tr>
                ...
                pageData.map((item, i) => {
                  ...
                  return (
                    <motion.tr
                      ...
                      className={cn(
                        "border-b border-incogni-border transition-colors last:border-b-0",
                        "hover:bg-incogni-surface/60",
                        isSelected && "bg-incogni-accent/8"
                      )}
                    >
                      {selectable && (
                      ...
                      {columns.map((col) => (
                        <td key={col.key} className="px-4 py-3 text-incogni-text">
                          {col.render
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={cn(
                ...
                page === 0
                  ? "text-incogni-muted/40 cursor-not-allowed"
