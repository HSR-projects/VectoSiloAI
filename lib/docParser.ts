 ...
 * document body wrapped in a single `<incogni-doc>…</incogni-doc>` block containing
...
const DOC_BLOCK_RE = /<incogni-doc(?:\s+title=["']([^"']*)["'])?\s*>([\s\S]*?)<\/incogni-doc>/i;
const DOC_OPEN_RE = /<incogni-doc(?:\s+title=["']([^"']*)["'])?\s*>/i;
...
export function hasDocSyntax(text: string): boolean {
  return DOC_DIRECTIVE_RE.test(text) || /<incogni-doc/i.test(text);
}
