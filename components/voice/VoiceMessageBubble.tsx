  ...
  return (
    <div className={cn(
      ...
      <button
        ...
        className={cn(
          ...
          align === "right"
            ...
            : "bg-incogni-accent/15 text-incogni-accent hover:bg-incogni-accent/25"
        )}
      >
        {playing ? (
        ...
        {bars.current.map((h, i) => (
          <div
            ...
            className={cn(
              ...
              compact ? "" : "min-h-[4px]",
              i <= activeBar && playing
                ? align === "right" ? "bg-white" : "bg-incogni-accent"
                : align === "right" ? "bg-white/30" : "bg-incogni-text/20"
            )}
            style={{
              height: compact
                ? `${Math.max(4, h * 24)}px`
      ...
      <span className={cn(
        ...
        align === "right" ? "text-white/70" : "text-incogni-muted"
      )}>
  ...
  return (
    <div className="flex h-14 items-center gap-2">
      <div className={cn(
        ...
        align === "right" ? "bg-white/10" : "bg-incogni-accent/10"
      )}>
        <Loader2 className="h-4 w-4 animate-spin text-incogni-muted" />
      </div>
      <div className="flex flex-1 items-center gap-1 animate-pulse">
        {Array.from({ length: 20 }).map((_, i) => (
          <div
            key={i}
            className="flex-1 rounded-full bg-incogni-text/10"
            style={{ height: `${Math.max(4, (0.15 + Math.random() * 0.85) * 36)}px` }}
          />
        ))}
      </div>
      <span className="w-8 shrink-0 text-xs text-incogni-muted">0:00</span>
    </div>
  );
}
