...
export function CapabilityCards({ onSelect }: { onSelect: (q: string) => void }) {
  return (
    <div className="mt-10">
      <p className="mb-4 text-center text-xs font-medium uppercase tracking-wider text-koda-muted">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {CAPABILITIES.map((c, i) => (
          <motion.button
            ...
            className="group flex flex-col items-start gap-2 rounded-xl border border-koda-border bg-koda-surface/50 p-4 text-left transition-all hover:border-koda-accent/30 hover:bg-koda-surface-2 hover:shadow-lg"
          >
            <span className="text-xl">{c.icon}</span>
            <span className="text-sm font-medium text-koda-text group-hover:text-koda-accent-soft">
              {c.title}
            </span>
            <span className="text-xs leading-relaxed text-koda-muted">{c.desc}</span>
          </motion.button>
        ))}
      </div>
    </div>
