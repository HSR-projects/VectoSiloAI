# "Continue with VectoSiloAI" — OAuth demo

A tiny sample website that integrates VectoSiloAI's OAuth provider, so you can test the
whole flow end to end. Zero dependencies — just Node 18+ (uses built-in `http` and
global `fetch`).

## Run it

1. **Register an app** in VectoSiloAI → [/developers](../app/developers) → **OAuth Apps**:
...
2. **Start the demo** with those credentials:
   ...
   ```bash
   cd oauth-demo
   CLIENT_ID=vectosilo_xxx CLIENT_SECRET=vectosilo_sk_xxx node server.mjs
   ```

   By default it talks to `http://localhost:3002` (your local VectoSiloAI dev server).
   ...
   > sure you're signed in to VectoSiloAI in the same browser.

3. Open **http://localhost:4567**, click **Continue with VectoSiloAI**, approve on the
...
| `VECTOSILO_BASE_URL` | `http://localhost:3002` | your VectoSiloAI instance |
