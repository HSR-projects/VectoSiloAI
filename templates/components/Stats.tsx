    ...
    <div className={className} {...rest}>
      ...
      {items.map((item, i) => (
        <motion.div
          ...
          className="flex flex-col gap-2 rounded-xl border border-incogni-border bg-incogni-surface p-5"
        >
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-incogni-muted">{item.label}</span>
            {item.icon && (
              <span className="text-incogni-accent">{item.icon}</span>
            )}
          </div>
