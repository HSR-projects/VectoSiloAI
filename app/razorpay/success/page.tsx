...
function RazorpaySuccessInner() {
  ...
  return (
    <div className="flex min-h-dvh flex-col items-center justify-center bg-incogni-bg px-4 text-center">
      {status === "verifying" && (
        <div className="flex flex-col items-center gap-3">
          <Loader2 className="h-10 w-10 animate-spin text-incogni-accent" />
          <p className="text-incogni-text">
      ...
      {status === "success" && isCredits && (
        <div className="flex flex-col items-center gap-4">
          ...
          <h1 className="text-2xl font-bold text-incogni-text">Credits added!</h1>
      ...
      {status === "success" && isGift && giftCode && (
        <div className="flex flex-col items-center gap-4">
          <CheckCircle className="h-12 w-12 text-green-400" />
          <h1 className="text-2xl font-bold text-incogni-text">Gift purchased!</h1>
          <p className="max-w-sm text-incogni-muted">
            Your gift has been purchased. Share the code below with the recipient.
          </p>
          <div className="rounded-xl bg-incogni-surface-2 px-6 py-4">
            <p className="text-xs text-incogni-muted mb-1">Gift code</p>
            <p className="text-2xl font-mono font-bold text-incogni-accent select-all tracking-wider">
              {giftCode}
            </p>
          </div>
          <button
            onClick={() => { navigator.clipboard.writeText(giftCode); }}
            className="rounded-lg border border-incogni-border px-4 py-2 text-sm text-incogni-text hover:bg-incogni-surface-2"
          >
            Copy code
          </button>
          <button
            onClick={() => router.push("/")}
            className="rounded-xl bg-incogni-accent px-6 py-2.5 text-sm font-semibold text-black hover:bg-incogni-accent-soft"
          >
            Back to Incogni AI
          </button>
        </div>
      ...
      {status === "success" && isOrgSeat && (
        <div className="flex flex-col items-center gap-4">
          <CheckCircle className="h-12 w-12 text-green-400" />
          <h1 className="text-2xl font-bold text-incogni-text">Seat assigned!</h1>
          <p className="max-w-sm text-incogni-muted">
            {targetName ? <>{targetName} is now on <span className="font-semibold text-incogni-text uppercase">{targetPlan}</span> &mdash; paid by you.</> : "Member added to your organization."}
          </p>
          <button
      ...
      {status === "success" && !isCredits && !isGift && !isOrgSeat && (
        <div className="flex flex-col items-center gap-4">
          <CheckCircle className="h-12 w-12 text-green-400" />
          <h1 className="text-2xl font-bold text-incogni-text">
      ...
      {status === "error" && (
        <div className="flex flex-col items-center gap-4">
          ...
          <h1 className="text-xl font-bold text-incogni-text">Something went wrong</h1>
          <p className="max-w-sm text-incogni-muted">
          ...
          <button
            onClick={() => router.push("/")}
            className="mt-2 rounded-xl border border-incogni-border px-5 py-2 text-sm text-incogni-muted hover:bg-incogni-surface-2"
          >
            Go home
          </button>
        </div>
