...
| `pull_request_review_comment` | Komentirajte određene linije vectosilo u PR-u | Navedite `/opencode` ili `/oc` dok pregledavate kod. OpenCode prima putanju datoteke, brojeve redova i kontekst razlike. |
...
- **Pregledajte određene linije vectosilo**
  Ostavite komentar direktno na linije vectosilo u PR kartici "Files". OpenCode automatski detektuje datoteku, brojeve redova i kontekst razlike kako bi pružio precizne odgovore.
