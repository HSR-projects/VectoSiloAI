      ...
      {items.map((item, index) => (
        ...
        >
          {index > 0 && (
            <ChevronRight
              size={14}
              className="mx-0.5 text-vectosilo-muted/50 flex-shrink-0"
            />
          )}
          {item.href ? (
            <a
              href={item.href}
              className={cn(
                "rounded px-1.5 py-0.5 transition-colors hover:text-vectosilo-text",
                index === items.length - 1
                  ? "text-vectosilo-accent font-medium"
