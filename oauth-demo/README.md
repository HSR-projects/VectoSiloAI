# "Continue with KodaAI" — OAuth demo

A tiny sample website that integrates KodaAI's OAuth provider, so you can test the
whole flow end to end. Zero dependencies — just Node 18+ (uses built-in `http` and
global `fetch`).

## Run it

1. **Register an app** in KodaAI → [/developers](../app/developers) → **OAuth Apps**:
...
2. **Start the demo** with those credentials:
   ...
   ```bash
   cd oauth-demo
   CLIENT_ID=koda_xxx CLIENT_SECRET=koda_sk_xxx node server.mjs
   ```

   By default it talks to `http://localhost:3002` (your local KodaAI dev server).
   To test against the live tunnel instead:

   ```bash
   KODA_BASE_URL=https://chat.hsrprojects.org \
   CLIENT_ID=koda_xxx CLIENT_SECRET=koda_sk_xxx node server.mjs
   ```

   > If you use the tunnel URL, register the redirect URI on that instance and make
   > sure you're signed in to KodaAI in the same browser.

3. Open **http://localhost:4567**, click **Continue with KodaAI**, approve on the
...
| `KODA_BASE_URL` | `http://localhost:3002` | your KodaAI instance |
