...
function incogniModelName(id: string, index: number): string {
  if (id === DEFAULT_MODEL || index === 0) return "IncogniAI Koder";
  return `IncogniAI Model ${index + 1}`;
}
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const models = raw.map((m, i) => ({
      id: m.name,
      name: incogniModelName(m.name, i),
      default: m.name === DEFAULT_MODEL,
    }));
    return NextResponse.json({ models, default: DEFAULT_MODEL });
  } catch {
    return NextResponse.json({
      models: [{ id: DEFAULT_MODEL, name: "IncogniAI Koder", default: true }],
